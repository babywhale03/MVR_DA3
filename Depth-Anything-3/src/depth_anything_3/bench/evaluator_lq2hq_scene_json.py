...
import math
from RAE.src.stage2.transport_lq2hq import create_transport, Sampler
...
class Evaluator:
    ...
    def __init__(
        ...
        if full_cfg.MVRM_EVAL.eval_method == 'w_mvrm' or full_cfg.MVRM_EVAL.eval_method == 'wo_mvrm_VAE':
    ...
    def infer(self, api, model_path, noise_generator, device) -> None:
        ...
        if self.full_cfg.MVRM_EVAL.eval_method == 'w_mvrm' or self.full_cfg.MVRM_EVAL.eval_method == 'wo_mvrm_VAE':
        ...
        for data, scene in tqdm(tasks, desc=f"Inference (GPU {self.gpu_id})"):
            ...
            self._save_scene_image_selection_json(data, scene, scene_data)
    ...
    def _get_scene_selection_json_path(self) -> str:
        """Path for scene-wise sampled image filename report."""
        filename = "scene_selected_images_by_max_frames.json"
        if self.total_gpus > 1:
            filename = f"scene_selected_images_by_max_frames_gpu{self.gpu_id}.json"
        return os.path.join(self.work_dir, filename)

    @staticmethod
    def _get_sampled_indices(num_frames: int, max_frames: int) -> list[int]:
        """Reproduce frame sampling indices used by _sample_frames."""
    ...
    def _save_scene_image_selection_json(self, data: str, scene: str, scene_data: Dict) -> None:
        """
        Save scene-wise selected image filenames for target max_frames values.

        Always records selections for max_frames in [4, 10, 30, 50], and also
        includes the currently configured self.max_frames if it is different.
        """
        if not hasattr(scene_data, "image_files"):
            return
        ...
        targets = [4, 10, 30, 50]
        if self.max_frames not in targets:
            targets.append(self.max_frames)
        targets = [v for v in targets if isinstance(v, int)]

        selected = {}
        for max_frames in sorted(set(targets)):
            sampled_indices = self._get_sampled_indices(num_frames, max_frames)
            selected[str(max_frames)] = [
                os.path.basename(str(image_files[i])) for i in sampled_indices if i < len(image_files)
            ]

        scene_payload = {
            "num_total_frames": int(num_frames),
            "selected_image_filenames": selected,
        }

        save_path = self._get_scene_selection_json_path()
        ...
        if data not in payload:
            payload[data] = {}
        payload[data][scene] = scene_payload
...
Examples:
  ...
  python -m depth_anything_3.bench.evaluator eval.print_only=true
    ...
    vae_model_path = config.model.vae_path if hasattr(config.model, "vae_path") else None
    ...
    if print_only:
        ...
        from depth_anything_3.api_lq2hq import DepthAnything3

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        api = DepthAnything3.from_pretrained(model_path, vae_model_path=vae_model_path)
        api = api.to(device)
