    ...
    def inference(
        ...
        if cfg.MVRM_EVAL.eval_method == 'w_mvrm':       
            ...
            model_kwargs={
                ...
                'val_lq_latent': lq_latent,
                'val_lq_cond': cfg.mvrm.lq_latent_cond
            }
            autocast_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
            with torch.no_grad():
                with torch.autocast(device_type=imgs.device.type, dtype=autocast_dtype):         
                    restored_samples = eval_sampler(pure_noise, denoiser.forward, **model_kwargs)[-1]     # b v n d # eval_sampler: class ode def sample() function
            ...
            if cfg.mvrm.lq_latent_cond == 'concat':
                restored_samples = restored_samples[..., :hq_latent.shape[-1]] 
            ...
            if cfg.mvrm.lq_latent_cond == 'concat':
                restored_samples_mvrm1 = restored_samples_mvrm1[..., :hq_latent.shape[-1]] 
            print(f"Restored samples shape: {restored_samples_mvrm1.shape}")
            ...
            if cfg.mvrm.lq_latent_cond == 'concat':
                restored_samples_mvrm2 = restored_samples_mvrm2[..., :hq_latent.shape[-1]] 
            print(f"Restored samples shape: {restored_samples_mvrm2.shape}")
        ...
        elif cfg.MVRM_EVAL.eval_method == 'wo_mvrm_VAE':             
            print('-'*70)
            print('WO_MVRM_VAE')
            print('-'*70)
            ...
            # (WO_MVRM_VAE) IR IMG FORWARD PASS
            print("VAE IMG FORWARD PASS")
            print(f"VAE scaling factor: {self.vae.config.scaling_factor} shift factor: {self.vae.config.shift_factor}")
            print(f"Original LQ image shape: {vae_lq_imgs.shape}")
            b, v, c, h, w = vae_lq_imgs.shape
            
            mean = torch.tensor([0.485, 0.456, 0.406], device=vae_lq_imgs.device).view(1, 1, 3, 1, 1)
            std = torch.tensor([0.229, 0.224, 0.225], device=vae_lq_imgs.device).view(1, 1, 3, 1, 1)
            vae_lq_imgs = vae_lq_imgs * std + mean

            vae_lq_imgs = rearrange(vae_lq_imgs, 'b v c h w -> (b v) c h w')
            vae_lq_imgs = vae_lq_imgs * 2.0 - 1.0
            lq_latents = self.vae.encode(vae_lq_imgs).latent_dist.sample() 
            if self.vae.config.shift_factor is not None:
                lq_latents = lq_latents - self.vae.config.shift_factor
            lq_latents = lq_latents * self.vae.config.scaling_factor
            lq_latents = rearrange(lq_latents, '(b v) d h w -> b v d h w', b=b, v=v)
            print(f"Encoded LQ latent shape: {lq_latents.shape}")

            noise_generator.manual_seed(42)
            pure_noise = torch.randn(lq_latents.shape, generator=noise_generator, device=imgs.device, dtype=torch.float32)
            
            model_kwargs={
                'guidance': cfg.mvrm.val.guidance,
                'val_lq_latent': lq_latents,
                'val_lq_cond': cfg.mvrm.lq_latent_cond
                # 'model_img_size': (val_h, val_w)
            }
            print(f"Starting evaluation sampler with pure noise shape: {pure_noise.shape}")
            autocast_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
            with torch.no_grad():
                with torch.autocast(device_type=imgs.device.type, dtype=autocast_dtype): 
                    restored_samples = eval_sampler(pure_noise, denoiser.forward, **model_kwargs)[-1]     # b v n d
            print(f"Restored samples shape: {restored_samples.shape}")
            ...
            with torch.no_grad():
                ...
                # ---- undo scaling ----
                restored_latent = restored_latent / self.vae.config.scaling_factor
                if self.vae.config.shift_factor is not None:
                    restored_latent = restored_latent + self.vae.config.shift_factor
                print(f"DEBUG: scale={self.vae.config.scaling_factor}, latent_max={restored_latent.max()}, latent_min={restored_latent.min()}")
                # decode
