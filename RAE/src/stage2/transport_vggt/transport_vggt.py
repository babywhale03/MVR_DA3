...
class Transport:
    ...
    def training_losses_mvrm(
        ...
        - model: backbone model; could be score, noise, or velocity
        - clean_img: [B, S, 3, H, W]
        - hq_latent: [B, S, num_tokens, token_dim], token_dim = 1024 if global_only
        - lq_latent: [B, S, num_tokens, token_dim] or None
        - model_kwargs: additional arguments for the model
        """
        if model_kwargs == None:
            model_kwargs = {}
        ...
        if lq_latent is not None:
            t, x0, x1 = self.sample(x1) # t: [B,], x0: [B, S, num_tokens, token_dim], x1: [B, S, num_tokens, token_dim]
            t, xt, ut = self.path_sampler.plan(t, x0, x1)
            xt = torch.cat([xt, lq_latent], dim=-1)
            model_output = model(clean_img, xt, t, **model_kwargs) # [B, S, num_tokens, out_channels]
            model_output = model_output[..., :1024]
        else:
            t, x0, x1 = self.sample(x1)
            t, xt, ut = self.path_sampler.plan(t, x0, x1)
            xt = torch.cat([xt, x1], dim=-1)
            model_output = model(clean_img, xt, t, **model_kwargs)
        # print(f"xt shape: {xt.shape}, model_output shape: {model_output.shape}, ut shape: {ut.shape}")
    ...
    def get_drift(
        ...
        def score_ode(clean_img, x, t, model, **model_kwargs):
            drift_mean, drift_var = self.path_sampler.compute_drift(x, t)
            model_output = model(clean_img, x, t, **model_kwargs)
            return (-drift_mean + drift_var * model_output) # by change of variable
        
        def noise_ode(clean_img, x, t, model, **model_kwargs):
            drift_mean, drift_var = self.path_sampler.compute_drift(x, t)
        ...
        # 2048 -> 2048 prediction mode
        def velocity_ode(clean_img, x, t, model, **model_kwargs):
            model_output = model(clean_img, x, t, **model_kwargs)
            return model_output
        
        # 2048 -> 1024 prediction mode
        # def velocity_ode(clean_img, x, t, model, **model_kwargs):
        #     cond = x[..., 1024:]
        #     model_output = model(clean_img, x, t, **model_kwargs)
        #     zero = torch.zeros_like(cond)
        #     drift = torch.cat([model_output, zero], dim=-1)
        #     return drift 
        ...
        def body_fn(clean_img, x, t, model, **model_kwargs):
            model_output = drift_fn(clean_img, x, t, model, **model_kwargs)
            return model_output
