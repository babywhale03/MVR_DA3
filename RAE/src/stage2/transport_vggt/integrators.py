...
class ode:
    ...
    def sample(self, clean_img, x, model, **model_kwargs):
        device = x[0].device if isinstance(x, tuple) else x.device

        def _fn(t, x):
            t_batch = th.ones(x[0].size(0) if isinstance(x, tuple) else x.size(0), device=device) * t
            return self.drift(clean_img, x, t_batch, model, **model_kwargs)

        t = self.t.to(device)
        x_curr = x

        for i in range(len(t) - 1):
            t_pair = th.tensor([t[i], t[i+1]], device=device)
            x_curr = odeint(
                _fn,
                x_curr,
                t_pair,
        ...
        x_curr = th.nan_to_num(x_curr, nan=0.0, posinf=1e4, neginf=-1e4)
        x_curr = th.clamp(x_curr, -1e4, 1e4)
