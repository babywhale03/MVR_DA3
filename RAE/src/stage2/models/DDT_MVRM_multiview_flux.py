...
class DiTwDDTHeadMVRM_Multiview_Flux(nn.Module):
    def __init__(
            self,
            input_size: int = 1,
        ...
        self.s_patch_size = patch_size
        self.x_patch_size = patch_size
    ...
    def forward(self, x, t, model_img_size=None, **kwargs):
        ...
        # time condition
        t = self.t_embedder(t)          # b*v d: [b, 1536]       
        c = nn.functional.silu(t)       # b*v d
        
        
        # encoder - patch embedding 
        x = rearrange(x, 'b v d h w -> (b v) d h w', b=b, v=v) # [b*v d 60 80]
        s = self.s_embedder(x)  # b*v d 60 80 -> b*v 30*40 d = [b 1200 1536]
        # print(f"s shape after patch embedding: {s.shape}")
        s = rearrange(s, '(b v) n d -> b v n d', b=b, v=v) # [b v n d]
