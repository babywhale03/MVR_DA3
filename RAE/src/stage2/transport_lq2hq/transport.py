...
class Transport:
    ...
    def training_losses_mvrm(
        ...
        x0 = x0 * cfg.mvrm.noiselvl + xcond
        t, xt, ut = self.path_sampler.plan(t, x0, x1)

        use_lq_cond = cfg.mvrm.use_lq_cond if hasattr(cfg.mvrm, 'use_lq_cond') else False
        if use_lq_cond:
    ...
    def get_drift(
        ...
        def velocity_ode(x, t, model, **model_kwargs):
            lq_latent = model_kwargs.get('lq_latent', None) 
            lq_cond = model_kwargs.get('lq_cond', 'addition')

            if lq_latent is not None:
                if lq_cond == 'addition':
                    x = x + lq_latent
                elif lq_cond == 'concat':
                    x = torch.concat([x, lq_latent], dim=-1)
