...
def load_checkpoint(
    ...
    if hasattr(model, 'module'):
        model_missing, model_unexpected = model.module.load_state_dict(checkpoint["model"], strict=False)
    else:
        model_missing, model_unexpected = model.load_state_dict(checkpoint["model"], strict=False)
...
def load_model_single(cfg, rank, device):
    models = {}
    processors = {}
    ...
    if cfg.stage_1.model == 'da3':
        ...
        encoder = VGGT.from_pretrained(cfg.stage_1.vggt.ckpt).to(device)
        encoder.eval()
        encoder_input_processor = None # Or the appropriate processor for VGGT
        encoder_output_processor = None
    elif cfg.stage_1.model == 'vae':
        from diffusers import AutoencoderKL
        vae = AutoencoderKL.from_pretrained(cfg.stage_1.vae.ckpt, subfolder="vae").to(device)
        vae.requires_grad_(False)
        models['vae'] = vae
    ...
    ##### Denoiser initialization 
    from stage2.models import Stage2ModelProtocol
    # Initialize the base model on the target device
    denoiser: Stage2ModelProtocol = instantiate_from_config(cfg.stage_2).to(device)         
    ...
    # Set main denoiser to train mode
    denoiser.requires_grad_(True) 
    denoiser.train()
    
    # --- Single GPU Change ---
    # We no longer wrap 'denoiser' in DDP().
    # We map both keys to the same model object to avoid breaking downstream code 
    # that might expect 'ddp_denoiser' or 'denoiser.module'.
    
    models['ema_denoiser'] = ema_denoiser
    models['ddp_denoiser'] = denoiser  
    models['denoiser'] = denoiser # No .module attribute needed here
